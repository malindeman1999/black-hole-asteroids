...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PyVista sky debug: textured sky + solved trajectories + observer vectors.")
...
def main() -> None:
    ...
    verts = np.asarray(data["vertices_m"], dtype=float)
    faces = np.asarray(data["faces"], dtype=np.int32)
    uv = np.asarray(data["uv"], dtype=np.float32)

    observer = np.asarray([float(args.observer_rs) * rs, 0.0, 0.0], dtype=float)
    bh = SchwarzschildBlackHole.from_diameter_light_seconds((2.0 * rs) / C).with_quality(str(args.quality))
    arr_p, arr_m, ok_p, ok_m = _interp_row(data, r_b=float(np.linalg.norm(observer)))

    rng = np.random.default_rng(int(args.seed))
    vidx = int(rng.integers(0, verts.shape[0]))
    a = np.asarray(verts[vidx], dtype=float)
    rr = bh.find_two_shortest_geodesics(a, observer, a_before_b=True, use_gpu=False)
    ...
    p = pv.Plotter(window_size=(1300, 850))
    p.set_background("black")
    p.add_mesh(
        mesh,
        texture=tex,
        opacity=float(np.clip(args.sky_opacity, 0.0, 1.0)),
    ...
    p.add_mesh(pv.Sphere(radius=float(np.linalg.norm(a)), center=(0.0, 0.0, 0.0)), style="wireframe", color="#3a6ea5", opacity=0.2)
    ...
    text_lines = [f"vertex={vidx}", f"observer={args.observer_rs:.2f} Rs"]
    for direction, color in ((+1, "#4cc9ff"), (-1, "#ffb17a")):
        path = next((pp for pp in rr.paths if int(pp.direction) == int(direction)), None)
        if path is None:
            continue
        xyz = _path_xyz_for_branch(a=a, b=observer, bh=bh, path=path)
        if xyz is not None:
            spl = pv.Spline(xyz.astype(np.float32), n_points=xyz.shape[0])
            p.add_mesh(spl, color=color, line_width=3)
        solve_arr = _arrival_dir_from_path(a=a, b=observer, rs=rs, path=path)
        p.add_arrows(observer.reshape(1, 3), (-solve_arr).reshape(1, 3), mag=vec_scale, color="#b455ff")

        pre_arr = arr_p[vidx] if direction == +1 else arr_m[vidx]
        ok = bool(ok_p[vidx]) if direction == +1 else bool(ok_m[vidx])
        if ok and np.all(np.isfinite(pre_arr)):
            p.add_arrows(
                observer.reshape(1, 3),
    ...
    p.add_text("\n".join(text_lines), font_size=10, color="white")
